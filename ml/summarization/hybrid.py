...
def compute_hybrid_scores(sentences, title, ner_results, tfidf_mat, vectorizer):
    ...
    ent_densities = np.array(
        [
            len(res.get("entities", [])) / len(res.get("tokens", [1]))
            if len(res.get("tokens", [])) > 0
            else 0
...
def predict_and_summarize(
    text, title=None, compression_ratio=0.3, stream=False, progress_callback=None
):
    """
    ...
    def _generator():
        ...
        try:
            ...
            logger.info("Hybrid summarization completed in %.3fs", elapsed)
            yield {
                "step": 4,
                "result": {
                    "summary": summary,
