...
def parse_uploaded_file_content(
    content_str: str, request: Request | None = None
) -> tuple[str, str]:
...
def validate_text_content(text: str, min_words: int = 10, request: Request | None = None) -> bool:
    ...
    if word_count < min_words:
        raise ValueError(
            lang(
                request,
                "Text is too short for summarization. Please provide at least %(min_words)d words (currently: %(word_count)d words).",
            )
    ...
    if len(meaningful_words) < min_words // 2:
        raise ValueError(
            lang(
                request,
                "Text contains insufficient meaningful content. Please provide more substantial text for summarization.",
            )
        )
    return True
