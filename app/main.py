...
# Register User Middleware FIRST (so it sits inside SessionMiddleware)
app.add_middleware(BaseHTTPMiddleware, dispatch=user_and_messages_middleware)

# Register SessionMiddleware SECOND (so it executes FIRST on incoming requests)
app.add_middleware(
    SessionMiddleware,
...
def _date_filter(value, format_str: str = "%b %d, %Y"):
