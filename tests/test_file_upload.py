...
def test_upload_valid_template_file(client: TestClient):
    ...
    files = {"file": ("template_sample.txt", io.BytesIO(file_content.encode("utf-8")), "text/plain")}
    data = {
        "method": "hybrid",
...
def test_upload_invalid_template_format(client: TestClient):
    ...
    invalid_content = "Ini adalah teks biasa tanpa marker TITLE atau TEXT yang disyaratkan oleh template sistem."
    files = {"file": ("invalid_format.txt", io.BytesIO(invalid_content.encode("utf-8")), "text/plain")}
    data = {"method": "hybrid"}
    response = client.post("/summarize/", data=data, files=files)
    assert response.status_code in (200, 400)
    assert "Format file tidak sesuai" in response.text or "template" in response.text.lower() or "alert-danger" in response.text
...
def test_upload_non_utf8_binary_file(client: TestClient):
    ...
    response = client.post("/summarize/", data=data, files=files)
    assert response.status_code in (200, 400)
    assert "UTF-8" in response.text or "Unable to read file" in response.text or "alert-danger" in response.text
...
def test_upload_authenticated_persists_summary(auth_client: TestClient, db_session: Session, test_user: User):
...
def test_upload_large_multiparagraph_document(client: TestClient):
    ...
    file_content = f"TITLE=Program Imunisasi Nasional\n\nTEXT={paragraph1}\n\n{paragraph2}\n\n{paragraph3}"
