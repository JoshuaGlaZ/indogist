...
def test_charts_rendered_with_context_data(client: TestClient):
    ...
    response = client.get("/charts/")
    assert response.status_code == 200
    assert "Model Training Convergence" in response.text or "Training History" in response.text or "data:image/png;base64," in response.text
    assert "Detailed Performance Classification" in response.text or "Metric Class" in response.text or "Precision" in response.text
